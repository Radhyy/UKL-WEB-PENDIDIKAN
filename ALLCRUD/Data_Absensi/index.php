...
$data = $koneksi->query("SELECT * FROM kehadiran");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><?= $row['status']; ?></td>
          <td>
            <a href="edit.php?id=<?= $row['id_absensi'] ?>" class="icon-btn" title="Edit"><i class="fas fa-edit"></i></a>
            <a href="hapus.php?id=<?= $row['id_absensi'] ?>" class="icon-btn delete" title="Hapus" onclick="return confirm('Hapus data ini?')"><i class="fas fa-trash-alt"></i></a>
          </td>
        </tr>
        <?php endwhile; ?>
      </tbody>
